function Write-SpectreCalendar2 {
<#
    .SYNOPSIS
    ...
    if ($Events) {
        foreach ($event1 in $events.GetEnumerator()) {
            # Calendar events don't appear to support Culture.
            $eventDate = $event1.Name -as [datetime]
            $calendar = [Spectre.Console.CalendarExtensions]::AddCalendarEvent($calendar, $event1.Value, $eventDate.Year, $eventDate.Month, $eventDate.Day)
        }
...
# Example usage:
Write-SpectreCalendar2 -Date 2024-07-01 -Events @{'2024-07-10' = 'Beach time!'; '2024-07-20' = 'Barbecue' }  